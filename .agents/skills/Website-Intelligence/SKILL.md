...
1. **If using Codex with MCP**: The user needs to add Firecrawl as an MCP server in their
   Codex settings. Guide them to add it via the settings UI or `settings.json`:
   ```json
   {
     "mcpServers": {
       "firecrawl": {
   ...
   ```
   After adding, they'll need to restart Codex for the MCP server to connect.
